...
/**
*load with:
./interactive_face_detection_demo -m /opt/intel/computer_vision_sdk_2018.4.420/deployment_tools/intel_models/face-detection-retail-0004/FP32/face-detection-retail-0004.xml -m_ag /opt/intel/computer_vision_sdk_2018.4.420/deployment_tools/intel_models/age-gender-recognition-retail-0013/FP32/age-gender-recognition-retail-0013.xml -m_em /opt/intel/computer_vision_sdk_2018.4.420/deployment_tools/intel_models/emotions-recognition-retail-0003/FP32/emotions-recognition-retail-0003.xml -m_hp /opt/intel/computer_vision_sdk_2018.4.420/deployment_tools/intel_models/head-pose-estimation-adas-0001/FP32/head-pose-estimation-adas-0001.xml -i cam 

*/
#include < gflags / gflags.h > #include < functional > #include < iostream > #include < fstream > #include < random > #include < memory > #include < chrono > #include < vector > #include < string > #include < utility > #include < algorithm > #include < iterator > #include < map >

  #include < inference_engine.hpp >

  #include < samples / common.hpp > #include < samples / slog.hpp >

  #include "interactive_face_detection.hpp"#include "detectors.hpp"

#include < ie_iextension.h > #include < ext_list.hpp >
  ...
  try {
    ...
    for (auto && option: cmdOptions) {
      ...
      if ((deviceName.find("CPU") != std::string::npos)) {
        ...
        plugin.SetConfig({
          {
            PluginConfigParams::KEY_CONFIG_FILE, FLAGS_c
          }
        });
      }
    ...
    while (true) {
      ...
      if (headPoseDetector.enabled() && 0 < headPoseDetector.maxBatch) {
        if (true) {
          ...
          if (headPoseDetector[0].angle_y <= yawradian && headPoseDetector[0].angle_y >= (-1.0 f * yawradian)) {
            timer.finish("reset");
            current_time_watched = current_time_watched + timer["reset"].getSmoothedDuration();
            timer.start("watch");
            if (current_time_watched > max_time_watched) {
              max_time_watched = current_time_watched;
            }
          } else {
            timer.finish("reset");
      ...
      prop_happy = ((1.0 f * happy_count) / emotion_count);
